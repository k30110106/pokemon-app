...
async def get_all_pokemons(
    ...
    pokemon_type: Optional[str] = Query(None, alias="type") # 외부에서는 'type'으로 받고 내부선 'pokemon_type'으로 사용
):
    query = {}
    if name:
    ...
    if pokemon_type:
        # MongoDB에서 types 배열 안에 해당 문자열이 포함되어 있는지 확인합니다.
        # 예: { "types": ["Grass", "Poison"] } 데이터에서 "Grass"로 검색 시 매칭됨
        query["types"] = pokemon_type
