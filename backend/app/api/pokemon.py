...
# [GET] /pokemon 주소로 요청이 오면 실행됩니다. 포켓몬 목록을 페이징 처리하여 반환합니다. 이름과 타입으로 필터링할 수 있습니다.
...
async def get_all_pokemons(
    ...
    cursor = db.pokemon.find(query, {"_id": 0}).sort("id", 1).skip(offset).limit(limit)
...
# [GET] /pokemon/{pokemon_id} 주소로 요청이 오면 실행됩니다. URL 경로에서 포켓몬 ID를 받아 해당 포켓몬의 상세 정보를 반환합니다.
